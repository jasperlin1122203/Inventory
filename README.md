# Inventory Management Simulator - Intern Project

> The project architecture and interfaces are complete. Interns need to complete the implementation based on TODO comments and documentation.
...
This project provides a complete architecture and interface definitions for an inventory management system. Your task is to implement the core business logic marked with `TODO` comments in the following components:

- **Service Layer**: `SimulationClock`, `OrderManager`, `InventoryManager`, `OrderInjector`, `OrderCSVReader`, `SimulationRunner`, `InventoryInitializer`
- **Controller Layer**: `OrderController`, `InventoryController`

All implementation code has been replaced with `UnsupportedOperationException` and TODO comments. Please refer to the code comments and documentation to understand the requirements and implement the functionality.
...
- 🔨 **SimulationClock**: Manages simulation time with configurable time ranges and speed acceleration *(Implementation Required)*
- 🔨 **Order Injector**: Reads orders from CSV files and injects them sequentially based on simulation time *(Implementation Required)*
- 🔨 **Order Manager**: Receives orders, checks inventory, and processes orders *(Implementation Required)*
- 🔨 **Inventory Manager**: Manages inventory, reservations, deductions, and automatic replenishment *(Implementation Required)*
...
> **Note**: Items marked with 🔨 require implementation. Items marked with ✅ are already configured and ready to use.
...
2. **Understand the Architecture**
   - Review the project structure and documentation in `docs/` folder
   - Read `docs/SYSTEM_DESIGN.md` for system architecture
   - Review `docs/IMPLEMENTATION_GUIDE.md` for implementation guidance

3. **Start Implementing**
   - Open the project in your IDE
   - Look for `TODO` comments in Service and Controller classes
   - Each method has detailed TODO comments explaining what needs to be implemented
   - Start with `SimulationClock` as it's the foundation

4. **Test Your Implementation**
   - Start RabbitMQ: `docker-compose up -d rabbitmq`
   - Run the application: `mvn spring-boot:run`
   - Test the APIs and verify the simulation runs correctly
